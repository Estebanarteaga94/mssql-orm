...
### Navigation Follow-Up Work

- Navigation properties are implemented in the `0.1.0` surface as explicit
  metadata, query joins, eager loading, and explicit collection loading. The
  remaining follow-up work is limited to features intentionally deferred below.
...
- Public query builder with:
  ...
  - navigation join inference (`try_inner_join_navigation`,
    `try_left_join_navigation`, and `_as` variants)
...
- Navigation properties with:
  ...
  - aliased table support in the query AST and SQL Server compiler for
    repeated joins, self-joins, and inferred navigation joins;
  - explicit navigation joins through `try_inner_join_navigation(...)`,
    `try_left_join_navigation(...)`, and `_as` variants without replacing
    manual joins;
  ...
  - policy filters for included `tenant` and default `soft_delete` entities
...
- Direct `many_to_many` navigation is not available; use an explicit join
  entity until relationship-update semantics are stable.
- Automatic lazy loading is not available; lazy wrappers are state containers
  only and later loaders must remain explicit and visibly async.
- Split-query execution for `include_many(...)` is not available yet; the
  builder exposes `split_query()` but returns a clear error when executed.
- Navigation graph tracking is not stable: included or explicitly loaded
  related entities are not registered automatically, and relationship changes
  are not persisted by `save_changes()`.
