...
use mssql_orm::query::{CountQuery, Expr, Predicate, SelectQuery};

pub fn user_lists_page_query(owner_user_id: i64, page: PageRequest) -> SelectQuery {
    SelectQuery::from_entity::<TodoList>()
        ...
        .paginate(page.to_pagination())
}

pub fn list_items_page_query(owner_user_id: i64, list_id: i64, page: PageRequest) -> SelectQuery {
...
pub fn open_items_preview_query(list_id: i64, limit: u64) -> SelectQuery {
    SelectQuery::from_entity::<TodoItem>()
        .filter(
            TodoItem::list_id
                .eq(list_id)
        ...
        .order_by(TodoItem::position.asc())
        .paginate(mssql_orm::query::Pagination::new(0, limit))
}

pub fn open_items_count_query(list_id: i64) -> CountQuery {
...
mod tests {
    use super::{
        list_items_page_query, open_items_count_query, open_items_preview_query,
        user_lists_page_query,
    };
    use crate::domain::{TodoItem, TodoList};
    use mssql_orm::prelude::*;
    use mssql_orm::query::{
        CountQuery, Expr, JoinType, OrderBy, Pagination, Predicate, SelectQuery, SortDirection,
        TableRef,
    };
    use mssql_orm::sqlserver::SqlServerCompiler;

    #[test]
    fn user_lists_page_query_builds_expected_ast() {
        let query = user_lists_page_query(7, PageRequest::new(2, 10));
    ...
    #[test]
    fn list_items_page_query_builds_expected_ast_with_joins() {
        let query = list_items_page_query(42, 9, PageRequest::new(3, 25));
    ...
    #[test]
    fn open_items_preview_query_builds_zero_offset_preview() {
        let query = open_items_preview_query(5, 3);
    ...
    #[test]
    fn list_items_page_query_compiles_expected_sql_server_select() {
        let compiled = SqlServerCompiler::compile_select(&list_items_page_query(
            42,
            9,
            PageRequest::new(2, 5),
        ))
    ...
    #[test]
    fn open_items_count_query_compiles_expected_sql_server_count() {
        let compiled = SqlServerCompiler::compile_count(&open_items_count_query(9))
    ...
    #[test]
    fn open_items_count_query_builds_expected_ast() {
        let query = open_items_count_query(11);

        assert_eq!(
            query,
            CountQuery::from_entity::<TodoItem>().filter(Predicate::and(vec![
                Predicate::eq(
                    Expr::from(TodoItem::list_id),
