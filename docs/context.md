...
1. Ejecutar `Etapa 16+: Definir cómo consultar entidades con soft_delete: por defecto las queries de entidades con la política deben excluir filas borradas lógicamente, y debe existir una API explícita para incluir o consultar solo eliminadas sin afectar entidades que no declaran soft_delete`.
2. Ejecutar `Etapa 16+: Definir cómo soft_delete obtiene valores runtime para columnas como deleted_at, deleted_by o is_deleted sin acoplar core a contexto por request ni duplicar la lógica actual de update`.
3. Mantener `tenant` como backlog `Etapa 16+` hasta diseñar contratos separados.
