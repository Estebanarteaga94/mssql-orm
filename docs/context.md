...
La Etapa 12 quedó cerrada con surface, persistencia, cobertura y límites documentados para el change tracking experimental. La Etapa 13 ya quedó cerrada también en migraciones avanzadas: índices compuestos, `computed columns`, foreign keys avanzadas, scripts idempotentes, `RenameColumn` explícito y `RenameTable` explícito ya están soportados dentro del pipeline de migraciones. La Etapa 14 también quedó cerrada: además de la surface operativa de producción (`timeouts`, `retry`, `tracing`, slow query, health, pool y wiring público desde pool`), el ejemplo web async `todo_app` ya tiene dominio, queries públicas, endpoints mínimos, wiring real con `MssqlPool` y validación reproducible contra SQL Server real. La Etapa 15 queda cerrada: ya están disponibles la documentación pública base (`README.md`), el quickstart reproducible, la consolidación de ejemplos/guías operativas, la guía `code-first` (`docs/code-first.md`), la guía de migraciones (`docs/migrations.md`), la guía del query builder público (`docs/query-builder.md`), la guía de transacciones runtime (`docs/transactions.md`), la guía de relaciones/joins (`docs/relationships.md`), el inventario de API pública (`docs/api.md`), el changelog inicial (`CHANGELOG.md`) y la validación final de release sobre workspace y `examples/todo-app`. A raíz de la desalineación previa con el plan maestro se añadió backlog técnico nuevo para cerrar el gap; ya quedaron implementadas piezas fundacionales: `mssql-orm` expone `MigrationModelSource` con `entity_metadata()` derivado desde `DbContext`, `mssql-orm-migrate` puede serializar/deserializar `ModelSnapshot` en JSON, `mssql-orm-cli migration add` puede consumir un snapshot actual explícito con `--model-snapshot <Path>`, la CLI puede invocar un binario exportador del consumidor con `--snapshot-bin <BinName> --manifest-path <Path>`, `migration add` ya carga además el `model_snapshot.json` de la última migración local cuando dispone de snapshot actual para dejar explícito el par `previous/current`, la propia CLI ya ejecuta internamente `snapshot -> diff -> MigrationOperation -> DDL SQL Server` para calcular el plan observable de la migración, escribe automáticamente ese SQL compilado en `up.sql` dejando `model_snapshot.json` versionado con el estado actual, genera `down.sql` cuando el plan completo es reversible con el payload disponible, bloquea por defecto cambios destructivos salvo `--allow-destructive`, `examples/todo-app` ya cuenta con exportador de snapshot y script reproducible para generar migración inicial, migración incremental no-op y `database_update.sql`, y `mssql-orm-cli database update --execute` ya puede aplicar el script acumulado contra SQL Server usando `--connection-string`, `DATABASE_URL` o `MSSQL_ORM_TEST_CONNECTION_STRING`. La Etapa 16 queda cerrada: `Entity Policies` está disponible como evolución code-first para auditoría de metadata/schema mediante `#[derive(AuditFields)]` y `#[orm(audit = Audit)]`, con documentación pública, ejemplo, migraciones y validación local mínima; autollenado runtime, `soft_delete`, `tenant` y `AuditProvider` quedan diferidos como backlog `Etapa 16+`. La evaluación de `concurrency = RowVersion` ya quedó cerrada: no debe implementarse como policy porque `#[orm(rowversion)]` ya es la API canónica y necesita un campo Rust visible para preservar `ConcurrencyConflict`. La evaluación de `soft_delete = SoftDelete` también queda cerrada: es una feature válida de roadmap, pero de alto riesgo porque hoy `DbSet::delete`, `entity.delete(&db)`, tracking y queries siguen anclados a borrado físico y a lecturas sin filtros implícitos. El siguiente paso correcto ya no es reevaluarla sino diseñar su semántica pública completa sobre las rutas existentes.
...
El alcance inicial también quedó acotado en `docs/entity-policies.md`: `audit = Audit` es la única policy que entra al MVP de implementación. `soft_delete`, `tenant`, `AuditProvider` y todo autollenado runtime quedan fuera del MVP porque cambian comportamiento de escritura, borrado, consultas, seguridad o tracking.

`mssql-orm-core` ya expone el contrato neutral `EntityPolicy` y el contenedor `EntityPolicyMetadata`. Una policy reusable declara `POLICY_NAME`, expone `COLUMN_NAMES` para validaciones constantes, devuelve `columns() -> &'static [ColumnMetadata]` y obtiene `metadata()` como helper. La crate pública reexporta ambos tipos desde `mssql-orm::prelude`; `AuditFields` y `#[orm(audit = Audit)]` ya usan ese contrato para validar y expandir columnas auditables dentro de la metadata de entidad.

La sintaxis MVP de auditoría quedó decidida en `docs/entity-policies.md`: `#[orm(audit = Audit)]` a nivel de entidad es la forma canónica. El lado derecho puede ser un path Rust visible desde el sitio del derive, como `Audit`, `crate::model::Audit` o `common::Audit`. Quedan fuera del MVP variantes implícitas, strings, llamadas runtime, providers y configuración inline de columnas dentro de la entidad.

El shape objetivo de `#[derive(AuditFields)]` también quedó definido en `docs/entity-policies.md`: debe aplicarse a structs con campos nombrados, cada campo genera `ColumnMetadata` normal, los tipos deben implementar `SqlTypeMapping` y `Option<T>` define nullability. Los atributos permitidos para campos auditables son `column`, `length`, `nullable`, `default_sql`, `sql_type`, `precision`, `scale`, `renamed_from`, `insertable` y `updatable`; quedan rechazados `primary_key`, `identity`, `computed_sql`, `rowversion`, `index`, `unique`, `foreign_key` y `on_delete`. Las flags `insertable` y `updatable` son metadata, no autollenado runtime.
...
`#[derive(Entity)]` ya acepta la sintaxis `#[orm(audit = Audit)]` a nivel de entidad y expande las columnas de `AuditFields` dentro de `EntityMetadata.columns`. El macro rechaza una segunda declaración `audit` en la misma entidad, evitando sobrescritura silenciosa. Antes de expandir columnas auditables, el macro genera aserciones constantes que fallan en compile-time si una columna auditable colisiona con una columna propia de la entidad; el mensaje nombra la columna duplicada y pide renombrar el campo de entidad o el campo de `AuditFields` mediante `#[orm(column = "...")]`. El orden estable actual es: primero columnas propias de la entidad en orden de declaración Rust y después columnas auditables en orden de declaración del struct `AuditFields`. Las entidades sin `audit` conservan el camino de metadata estática anterior.

La cobertura de metadata de `#[orm(audit = Audit)]` ya confirma explícitamente `schema`, `table`, primary key, columnas propias, columnas auditables, defaults, nullability, longitudes, tipos SQL Server, flags `insertable`/`updatable` y orden estable dentro de `crates/mssql-orm/tests/stage16_entity_policies.rs`. Esa cobertura no agregó comportamiento runtime: sigue validando que las columnas de policy llegan al modelo como `ColumnMetadata` normales.

`ModelSnapshot::from_entities(...)` ya está cubierto con entidades auditadas desde `crates/mssql-orm/tests/stage16_entity_policies.rs`: las columnas aportadas por `AuditFields` se convierten a `ColumnSnapshot` normales, preservan orden, tipos, defaults, nullability, `insertable`/`updatable`, longitudes y nombres renombrados, y sobreviven a `to_json_pretty()`/`from_json(...)` sin un pipeline especial para policies.

La creación de migración para una entidad nueva auditada ya está cubierta en `crates/mssql-orm/tests/stage16_audit_migrations.rs`. El test usa derives reales desde la crate pública y valida el flujo `EntityMetadata -> ModelSnapshot -> diff_schema_and_table_operations -> MigrationOperation -> SqlServerCompiler`, produciendo `CREATE TABLE [audit].[audited_entities]` con columnas propias y columnas auditables. Esto confirma que el pipeline de migraciones no requiere una ruta especial para `audit = Audit` cuando la tabla es nueva.

Activar `#[orm(audit = Audit)]` sobre una tabla existente también está cubierto en `crates/mssql-orm/tests/stage16_audit_migrations.rs`. El test compara una entidad previa sin auditoría contra la entidad auditada sobre el mismo schema/table y valida que `diff_column_operations(...)` emite solo operaciones `AddColumn` para `created_at`, `created_by_user_id`, `updated_at` y `updated_by`, preservando tipo, default SQL, nullability, longitud y flags de persistencia.

Quitar `#[orm(audit = Audit)]` sobre una tabla existente está cubierto como cambio destructivo. `crates/mssql-orm/tests/stage16_audit_migrations.rs` valida que el diff produce `DropColumn` para cada columna auditable, y `crates/mssql-orm-cli/src/main.rs` valida que `migration add` bloquea por defecto ese plan cuando el snapshot previo tiene auditoría y el snapshot actual ya no la tiene. El bloqueo reutiliza la detección destructiva existente y reporta la primera operación como `DropColumn audit.audited_entities.created_at`.

El DDL SQL Server para columnas auditables está cubierto por snapshot en `crates/mssql-orm-sqlserver/tests/migration_snapshots.rs`. El snapshot `migration_snapshots__audit_column_migration_sql.snap` fija `ALTER TABLE ... ADD` para `created_at datetime2 NOT NULL DEFAULT SYSUTCDATETIME()`, `created_by_user_id bigint NULL`, `updated_at datetime2 NULL DEFAULT SYSUTCDATETIME()` y `updated_by nvarchar(120) NULL`, manteniendo la generación SQL dentro de la crate `mssql-orm-sqlserver`.

En este corte, las columnas auditables son metadata/schema: participan como `ColumnMetadata` normales, pero no generan campos Rust visibles en la entidad ni símbolos asociados como `Todo::created_at`. Esta decisión quedó fijada con un fixture `trybuild` negativo: si `created_at` viene de `AuditFields`, `AuditedEntity::created_at` no existe. `FromRow` generado para entidades auditadas sigue materializando solo los campos Rust reales de la entidad: no exige columnas auditables en la fila y las ignora cuando vienen presentes. `EntityPersist`, `DbSet::insert`, `DbSet::update`, Active Record y `save_changes` siguen trabajando solo con los campos reales de la entidad; el autollenado runtime queda diferido.

`examples/todo-app` ya incluye una muestra real de Entity Policies sin tocar las rutas HTTP existentes: `TodoAudit` deriva `AuditFields`, `AuditEvent` declara `#[orm(table = "audit_events", schema = "todo", audit = TodoAudit)]` y `TodoAppDbContext` expone `audit_events: DbSet<AuditEvent>`. El fixture `smoke_setup.sql` crea `todo.audit_events` sin datos para mantener el smoke alineado con el modelo sin afectar endpoints. La validación local confirmó metadata, compilación, tests del ejemplo y exportación de snapshot con la tabla auditada.
...
La API publica queda concentrada en la crate raiz mediante `mssql_orm::prelude::*`, que reexporta `AuditFields`, `EntityPolicy` y los contratos necesarios para el caso valido. Las extensiones `AuditProvider`, `soft_delete` y `tenant` quedan como `Etapa 16+`; no deben implementarse sin diseno especifico porque cambian rutas de escritura, borrado, consulta, tracking, transacciones o seguridad.

`docs/entity-policies.md` ya contiene un diseno conceptual de `AuditProvider` para autollenado futuro. Ese diseno separa metadata de runtime: `audit = Audit` sigue siendo la fuente de columnas, mientras `AuditProvider` solo resolveria valores para columnas existentes durante insert/update. El contrato conceptual incluye `now`, usuario actual, valores por request, `AuditContext`, integracion con `DbContext` y herencia de provider/valores dentro de `db.transaction(...)`.

La revision posterior del diseno dejo explicito el acoplamiento correcto con la implementacion actual: el autollenado futuro debe engancharse en la capa publica de persistencia donde convergen `DbSet::insert`, `DbSet::update`, Active Record y `save_changes()`, aprovechando las rutas internas de valores crudos antes de construir `InsertQuery`/`UpdateQuery`. No debe moverse a derives, `mssql-orm-query`, `mssql-orm-sqlserver` ni `mssql-orm-tiberius`.

La mutacion futura de `Vec<ColumnValue>` por `AuditProvider` ya quedo definida en `docs/entity-policies.md`. Debe existir una unica transformacion interna en `mssql-orm` que reciba valores explicitos producidos por `Insertable`, `Changeset` o `EntityPersist`, aplique `AuditOperation::Insert` o `AuditOperation::Update`, complete solo columnas auditables faltantes, respete `insertable`/`updatable`, detecte duplicados como error y preserve la precedencia de valores explicitos del usuario. `DbSet::insert`, `DbSet::update`, Active Record y `save_changes()` deben converger en esa transformacion mediante `insert_entity_values(...)` y `update_entity_values_by_sql_value(...)`; `RawInsertable` y `RawChangeset` quedan como adaptadores mecanicos para valores ya normalizados.

La implementacion runtime de `AuditProvider` no debe inferir auditoria por nombres como `created_at` o `updated_by`. Como `EntityMetadata.columns` no conserva hoy el origen de cada columna, una implementacion posterior necesitara un contrato auxiliar de runtime generado por `#[orm(audit = Audit)]` para exponer el slice de columnas auditables de la entidad. Ese contrato no debe cambiar snapshots, diff ni DDL; solo permite que la crate publica sepa que columnas puede autollenar.

La policy `audit = Audit` cubre tambien los casos que solo necesitan `created_at` y `updated_at`: el usuario puede definir un struct de auditoria reducido con esas columnas. Por decision de roadmap, no se mantiene una policy separada para ese caso en la documentacion viva ni en el backlog.

La concurrencia optimista queda fuera del mecanismo de `Entity Policies`. La forma canonica sigue siendo `#[orm(rowversion)]` sobre un campo `Vec<u8>` visible en la entidad, porque el token debe materializarse desde SQL Server y viajar por `EntityPersist::concurrency_token()`, `Changeset::concurrency_token()`, Active Record y `save_changes()`. Una policy oculta `concurrency = RowVersion` no tendria campo Rust donde guardar el token y forzaria un segundo pipeline o una semantica incompleta. `OrmError::ConcurrencyConflict` debe seguir saliendo de las rutas actuales cuando el token no coincide y la primary key todavia existe.

Para `soft_delete`, la evaluación ya dejó fijado que no es una policy pequeña de metadata. El diseño ya quedó bajado a reglas operativas concretas: `DeleteQuery` y `compile_delete(...)` conservan significado de borrado físico, y el branching semántico debe ocurrir en `DbSet`/Active Record/tracking; para entidades con `soft_delete`, esas rutas deben converger en `UpdateQuery` con predicate por PK + `rowversion` y assignments de borrado lógico. Esa parte ya tiene implementación real: `DbSet::delete(...)`, `delete_by_sql_value(...)`, `delete_tracked_by_sql_value(...)`, `entity.delete(&db)` y `save_tracked_deleted()` ya resuelven internamente un `UPDATE` cuando la entidad declara `soft_delete`, mientras que entidades normales siguen compilando `DELETE`. La detección de `ConcurrencyConflict` también quedó alineada en esa misma ruta compartida cuando hay `rowversion`. El contrato runtime ya está cableado al contexto público: `SharedConnection` puede transportar `SoftDeleteProvider` y `SoftDeleteRequestValues`, y el derive `DbContext` expone helpers `with_soft_delete_provider(...)`, `with_soft_delete_request_values(...)` y `clear_soft_delete_request_values()` preservando la misma conexión compartida y el mismo `tracking_registry`. Eso permite que `delete`, Active Record y change tracking reutilicen el provider sin wiring manual por `DbSet`. La visibilidad de lectura también quedó implementada en `DbSetQuery`: entidades con `soft_delete` nacen en modo `ActiveOnly`, `with_deleted()` elimina el filtro implícito y `only_deleted()` lo invierte para la entidad raíz. El filtro se materializa recién al compilar/ejecutar el `SelectQuery`, sin contaminar el AST base. La convención observable actual es conservadora y explícita: la **primera** columna declarada por la policy controla visibilidad; si esa columna es nullable se usa `IS NULL` / `IS NOT NULL`, y si es `BIT` se usa `false` / `true`. Los joins explícitos siguen conservadores: el filtro automático aplica solo a la entidad raíz `E`, no a cada entidad unida manualmente. También quedaron aisladas las rutas internas sin filtro implícito: `DbSet` mantiene helpers internos para existencia/lectura real usados en `ConcurrencyConflict`, `save()` y checks post-write, sin exponer un bypass público adicional en la crate raíz ni en `DbSetQuery`. `soft_delete` también quedó integrado al pipeline de schema sin ruta especial: `ModelSnapshot::from_entities(...)` captura sus columnas como `ColumnSnapshot` ordinarias, `diff_column_operations(...)` produce `AddColumn`/`DropColumn` al activar o quitar la policy y `mssql-orm-sqlserver` compila el DDL mediante el mismo camino de columnas usado por el resto del modelo. Con ese estado, el siguiente bloque correcto es cerrar cobertura integral de runtime/CRUD/Active Record/tracking/ConcurrencyConflict/migraciones para demostrar que ninguna ruta pública sigue haciendo borrado físico por accidente.
...
- `CHANGELOG.md` ya existe como changelog inicial para `0.1.0 - Unreleased`, con surface disponible, Entity Policies, exclusiones explícitas, validación conocida y enlaces de documentación.
- `docs/api.md` ya existe como inventario mínimo de la surface pública publicada por la crate raíz. Distingue `prelude` para consumidores normales de los módulos avanzados reexportados (`core`, `query`, `migrate`, `sqlserver`, `tiberius`) y enumera exclusiones explícitas del release actual.
...
- `docs/transactions.md` ya existe como guía operativa de transacciones runtime. Documenta `db.transaction(|tx| async move { ... })`, commit en `Ok`, rollback en `Err`, timeouts, tracing, retry deshabilitado dentro de transacción, ausencia de savepoints y el límite actual de no usar `db.transaction(...)` sobre contextos creados desde `from_pool(...)` hasta pinnear una conexión física durante todo el closure.
- `docs/relationships.md` ya existe como guía pública de relaciones y joins. Documenta la sintaxis estructurada `#[orm(foreign_key(entity = User, column = id))]`, la sintaxis string legacy, `on_delete`, helpers de `ForeignKeyMetadata`, migraciones/DDL derivadas y el uso explícito de `inner_join`/`left_join`, dejando claro que no hay navigation properties ni joins inferidos automáticamente.
...
- `mssql-orm-cli migration add` ahora intenta invertir el plan completo en orden inverso y escribe `down.sql` generado cuando todas las operaciones conservan payload suficiente. Las operaciones reversibles actuales incluyen creación de schema/table/column/index/foreign key, renombres y `AlterColumn`; operaciones como `DropTable`, `DropColumn`, `DropIndex` y `DropForeignKey` quedan manuales porque el payload actual no reconstruye el objeto eliminado.
...
- El artefacto editable MVP de migración queda definido como `up.sql`, `down.sql` y `model_snapshot.json`. La CLI lista esas rutas al crear una migración, `down.sql` queda como rollback revisable generado automáticamente solo cuando el plan es reversible y manual en los demás casos, y `migration.rs` se difiere explícitamente hasta diseñar una API Rust de migraciones que no duplique el pipeline actual.
...
- La validación real de `todo_app` ya quedó cerrada con fixture SQL reproducible, smoke HTTP manual y prueba ignorada de lectura contra `DATABASE_URL`; la validación final de release de Etapa 15 también quedó cerrada sobre workspace, ejemplo documentado, exportador de snapshot y script `migration_e2e.sh`.
