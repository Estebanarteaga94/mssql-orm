...
El alcance inicial también quedó acotado en `docs/entity-policies.md`: `audit = Audit` es la única policy que entra al MVP de implementación; `timestamps = Timestamps` queda reconocida como policy candidata de columnas generadas pero diferida hasta decidir si será policy separada, alias o convención sobre el mismo contrato. `soft_delete`, `tenant`, `AuditProvider` y todo autollenado runtime quedan fuera del MVP porque cambian comportamiento de escritura, borrado, consultas, seguridad o tracking.
...
`#[derive(Entity)]` ya acepta la sintaxis `#[orm(audit = Audit)]` a nivel de entidad y expande las columnas de `AuditFields` dentro de `EntityMetadata.columns`. El macro rechaza una segunda declaración `audit` en la misma entidad, evitando sobrescritura silenciosa y dejando preparado el contrato de colisiones para una futura segunda policy como `timestamps`. Antes de expandir columnas auditables, el macro genera aserciones constantes que fallan en compile-time si una columna auditable colisiona con una columna propia de la entidad; el mensaje nombra la columna duplicada y pide renombrar el campo de entidad o el campo de `AuditFields` mediante `#[orm(column = "...")]`. El orden estable actual es: primero columnas propias de la entidad en orden de declaración Rust y después columnas auditables en orden de declaración del struct `AuditFields`. Las entidades sin `audit` conservan el camino de metadata estática anterior.
...
`timestamps = Timestamps` ya fue evaluado como extension `Etapa 16+`. La decision vigente es no modelarlo como alias de `audit`: debe ser una policy separada de columnas generadas, definida por el usuario mediante un derive futuro como `#[derive(TimestampFields)]` y consumida con `#[orm(timestamps = Timestamps)]`. Debe reutilizar `EntityPolicy`, producir `ColumnMetadata` normales, no generar campos Rust visibles ni simbolos asociados, no activar autollenado runtime y validar colisiones contra columnas propias y contra `audit = Audit` antes de exponer metadata. El orden esperado de expansion futura es columnas propias, luego `audit`, luego `timestamps`, siempre con validaciones previas de colision. El backlog ya quedo dividido en subtareas de diseno, parser, derive, colisiones y cobertura.
...
1. Ejecutar `Etapa 16+: Diseñar TimestampFields como derive dedicado para timestamps = Timestamps, reutilizando EntityPolicy sin introducir alias implícito de audit`.
2. Mantener `concurrency = RowVersion`, `soft_delete` y `tenant` como backlog `Etapa 16+` hasta diseñar contratos separados.
