...
Continuar la Etapa 6 con la siguiente subtarea detallada: exponer paginación pública en `DbSetQuery` con un request explícito y estable, ahora que la crate pública ya soporta predicados, ordenamiento, `filter`/`order_by` y recorte básico con `limit`/`take`.
...
- `DbSetQuery<T>` ya encapsula un `SelectQuery` y soporta `filter`, `order_by`, `limit`, `take`, `all`, `first` y `count`, reutilizando `SqlServerCompiler`, `fetch_one` y `fetch_all` sin mover ejecución ni generación SQL fuera de sus crates.
...
1. Implementar `Etapa 6: Exponer paginación pública en DbSetQuery con request explícito y contrato estable`.
2. Continuar con composición lógica y pruebas específicas del query builder público, reutilizando `DbSetQuery<T>` y el AST de `mssql-orm-query` como base.
