...
- La CLI ya acepta `migration add <Name> --model-snapshot <Path>` como entrada explícita del snapshot actual del modelo. Esa ruta permite versionar snapshots reales mientras se implementa la carga automática del `DbContext` consumidor; sin el flag, `migration add` conserva el scaffold con snapshot vacío válido.
