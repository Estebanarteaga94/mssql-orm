...
Para `soft_delete`, la evaluación ya dejó fijado que no es una policy pequeña de metadata. El diseño ya quedó bajado a reglas operativas concretas: `DeleteQuery` y `compile_delete(...)` conservan significado de borrado físico, y el branching semántico debe ocurrir en `DbSet`/Active Record/tracking; para entidades con `soft_delete`, esas rutas deben converger en `UpdateQuery` con predicate por PK + `rowversion` y assignments de borrado lógico. Esa parte ya tiene implementación parcial real: `DbSet::delete(...)`, `delete_by_sql_value(...)`, `delete_tracked_by_sql_value(...)`, `entity.delete(&db)` y `save_tracked_deleted()` ya resuelven internamente un `UPDATE` cuando la entidad declara `soft_delete`, mientras que entidades normales siguen compilando `DELETE`. La detección de `ConcurrencyConflict` también quedó alineada en esa misma ruta compartida cuando hay `rowversion`. La semántica de consulta sigue pendiente: `DbSet::query()` y `find()` todavía no excluyen filas borradas lógicamente, pero el diseño ya fija que `DbSetQuery` deberá exponer `with_deleted()` y `only_deleted()` y que la visibilidad debe vivir en `DbSetQuery<E>` en vez de contaminar `SelectQuery`. Los joins explícitos siguen conservadores: el filtro automático deberá aplicar solo a la entidad raíz `E`, no a cada entidad unida manualmente. El contrato runtime también tiene soporte real en `mssql-orm`: la crate pública expone `SoftDeleteProvider`, `SoftDeleteContext`, `SoftDeleteOperation` y `SoftDeleteRequestValues`, además del helper interno que valida duplicados, updatability, nullability y columnas runtime requeridas antes de compilar el `UpdateQuery`. La limitación vigente es concreta: todavía no existe wiring público de `SoftDeleteProvider`/valores por request en `DbContext`, así que una entidad `soft_delete` cuya policy necesite valores runtime como `deleted_at` o `deleted_by` hoy falla de forma explícita antes de compilar SQL. Con ese estado, el siguiente bloque de trabajo correcto ya no es volver a tocar el branching de delete sino integrar el provider al contexto y luego avanzar con filtros automáticos de lectura, helpers internos, migraciones y cobertura.
...
1. Ejecutar `Etapa 16+: Integrar SoftDeleteProvider y/o SoftDeleteRequestValues al DbContext/DbSet para que delete, Active Record y change tracking puedan poblar columnas como deleted_at, deleted_by o is_deleted sin wiring manual interno`.
