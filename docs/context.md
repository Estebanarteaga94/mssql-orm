...
1. Ejecutar `Etapa 16+: Definir cómo AuditProvider debe modificar Vec<ColumnValue> en insert/update sin duplicar la lógica existente de Insertable, Changeset, EntityPersist, Active Record ni change tracking`.
2. Mantener `timestamps`, `concurrency = RowVersion`, `soft_delete` y `tenant` como backlog `Etapa 16+` hasta diseñar contratos separados.
