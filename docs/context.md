...
- `mssql-orm-cli migration add` ya ensambla internamente `diff_schema_and_table_operations`, `diff_column_operations`, `diff_relational_operations` y `SqlServerCompiler::compile_migration_operations(...)`. En esta etapa el resultado queda visible como `Planned operations` y `Compiled SQL statements`, pero todavía no se vuelca automáticamente a `up.sql`.
