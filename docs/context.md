...
- `docs/migrations.md` ya existe como guía operativa de migraciones y deja explícito que la CLI actual `mssql-orm-cli` scaffolda, lista, genera `up.sql` desde snapshots reales y genera el script de `database update`, pero no aplica SQL directamente.
...
- `examples/todo-app/src/bin/model_snapshot.rs` exporta el `ModelSnapshot` real de `TodoAppDbContext`, y `examples/todo-app/scripts/migration_e2e.sh` valida la generación automática de migración inicial, migración incremental no-op y script `database_update.sql` en un directorio temporal. La aplicación real con `sqlcmd` queda condicionada a configurar `MSSQL_ORM_SQLCMD_SERVER`, `MSSQL_ORM_SQLCMD_USER` y `MSSQL_ORM_SQLCMD_PASSWORD`.
