...
`mssql-orm-macros` ya implementa `#[derive(AuditFields)]`. El derive genera una implementación de `EntityPolicy` con `POLICY_NAME = "audit"` y expone los campos del struct como `ColumnMetadata` estático. La crate pública reexporta `AuditFields` desde `mssql_orm::prelude`. Esta pieza todavía no conecta esas columnas a una entidad: `#[derive(Entity)]` aún no acepta `#[orm(audit = Audit)]`.
...
`#[derive(Entity)]` ya acepta la sintaxis `#[orm(audit = Audit)]` a nivel de entidad y conserva sin cambios la metadata de entidades que no declaran auditoría. En este corte el atributo solo se parsea y se valida sintácticamente como path Rust; todavía no expande las columnas de `AuditFields` dentro de `EntityMetadata.columns`.
