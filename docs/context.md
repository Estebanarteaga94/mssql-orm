...
Para `soft_delete`, la evaluación ya dejó fijado que no es una policy pequeña de metadata. Hoy `DbSet::delete(...)`, `entity.delete(&db)`, `DbSet::remove_tracked(...)` y `save_changes()` siguen convergiendo en borrado físico, mientras `DbSet::query()` y `DbSetQuery` no aplican filtros implícitos. El diseño ya quedó bajado a reglas operativas concretas: `DeleteQuery` y `compile_delete(...)` conservan significado de borrado físico, y el branching semántico debe ocurrir en `DbSet`/Active Record/tracking; para entidades con `soft_delete`, esas rutas deben converger en `UpdateQuery` con predicate por PK + `rowversion` y assignments de borrado lógico. La detección de `ConcurrencyConflict` debe seguir funcionando con comprobaciones internas sin filtros implícitos de `soft_delete`. La semántica de consulta también quedó fijada: `DbSet::query()` y `find()` deben excluir por defecto filas borradas lógicamente para la entidad raíz, `DbSetQuery` debe exponer APIs visibles `with_deleted()` y `only_deleted()`, y la visibilidad debe vivir en `DbSetQuery<E>` en vez de contaminar `SelectQuery`. Los joins explícitos quedan conservadores en esta fase: el filtro automático solo aplica a la entidad raíz `E`, no a cada entidad unida manualmente. El contrato runtime también quedó definido a nivel conceptual: `soft_delete` necesita un provider separado de su metadata, responsable de mutar un `Vec<ColumnValue>` ya normalizado en la crate pública `mssql-orm`, sin inferir columnas por nombres mágicos ni acoplar `core` a contexto por request. Esa base ya tiene soporte parcial real: `#[derive(Entity)]` acepta `#[orm(soft_delete = SoftDelete)]`, expande sus columnas como metadata ordinaria y genera el contrato auxiliar `SoftDeleteEntity` para que el runtime futuro identifique la policy de la entidad. Aun no hay borrado lógico efectivo en `DbSet` ni filtros automáticos ejecutándose en runtime. Con ese diseño ya cerrado, el backlog fue descompuesto en subtareas de implementación concretas para macros, runtime, rutas de `DbSet`, visibilidad de lectura, helpers internos y cobertura.
...
1. Ejecutar `Etapa 16+: Implementar el contrato runtime de soft_delete en mssql-orm para producir y validar Vec<ColumnValue> de borrado lógico sin duplicar la lógica de update`.
