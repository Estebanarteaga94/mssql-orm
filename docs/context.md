...
`mssql-orm-macros` ya implementa `#[derive(AuditFields)]`. El derive genera una implementación de `EntityPolicy` con `POLICY_NAME = "audit"` y expone los campos del struct como `ColumnMetadata` estático. La crate pública reexporta `AuditFields` desde `mssql_orm::prelude`.
