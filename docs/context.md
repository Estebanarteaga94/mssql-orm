...
1. Mantener `AuditProvider`, `timestamps`, `concurrency = RowVersion`, `soft_delete` y `tenant` como backlog `Etapa 16+` hasta diseñar contratos separados.
2. Si se continúa con `Etapa 16+`, empezar por diseñar `AuditProvider` para autollenado futuro sin tocar todavía rutas de persistencia.
