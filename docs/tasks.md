...
## Pendientes
- [ ] Etapa 16+: Integrar `SoftDeleteProvider` y/o `SoftDeleteRequestValues` al `DbContext`/`DbSet` para que `delete`, Active Record y change tracking puedan poblar columnas como `deleted_at`, `deleted_by` o `is_deleted` sin wiring manual interno
